...
import { sessionCache, dynamicCache } from "../utils/CacheManager";
...
class ProfileApiService {
  ...
  async getProfile(userId: string): Promise<ApiResponse<ProfileData>> {
    try {
      ...
      const cachedProfile = sessionCache.get<ProfileData>(cacheKey);
      if (cachedProfile) {
        console.log(`🎯 Cache HIT: Profile data for ${userId}`);
        return {
          success: true,
          data: cachedProfile,
          timestamp: new Date().toISOString(),
      ...
      sessionCache.set(cacheKey, profileData, 30 * 60 * 1000);
    ...
    try {
      ...
      if (updateData.preferences) {
        const currentPreferences =
          userDoc.data().preferences || this.getDefaultPreferences();
  ...
  async recordAchievement(
    ...
    try {
      ...
      const existingAchievement = currentAchievements.find(
        (a: Achievement) => a.id === achievementData.id
      );
  ...
  async getAchievements(
    userId: string
  ): Promise<ApiResponse<{ unlocked: Achievement[]; available: any[] }>> {
    try {
      const userRef = doc(db, `${getBasePath()}/users/${userId}`);
      const userDoc = await getDoc(userRef);
  ...
  private async calculateDerivedStats(
    userId: string,
    userData: any
  ): Promise<UserStats> {
    ...
    const favoriteCategory = await this.calculateFavoriteCategory(
  ...
  private async calculateFavoriteCategory(
  ...
  private validateProfileUpdate(updateData: ProfileUpdateData): {
    isValid: boolean;
    message?: string;
  } {
    // Validate username
  ...
  private calculateAchievementRewards(achievement: Achievement): {
    coins: number;
    experience: number;
  } {
    const baseRewards = {
  ...
  private async getAvailableAchievements(
    ...
    const unlockedIds = (userData.achievements || []).map(
      (a: Achievement) => a.id
    );
