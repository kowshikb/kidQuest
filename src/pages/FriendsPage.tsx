    ...
    try {
      ...
      if (querySnapshot.empty) {
        showModal({
          ...
          message:
            "We couldn't find a champion with that ID. Check the ID and try again!",
          type: "error",
        });
        return;
      }
      ...
      try {
        await notificationsApi.createFriendRequestNotification(
          targetUserId,
          currentUser.uid,
          userProfile?.username || "Unknown Champion",
          docRef.id
        );
        console.log("✅ Friend request notification sent!");
      } catch (notificationError) {
        console.warn(
          "Could not send friend request notification:",
          notificationError
        );
      }
      ...
      showModal({
        ...
        message: `Your friend request has been sent to ${
          targetUserData.username || "the champion"
        } successfully!`,
        type: "success",
      });
  ...
  const acceptFriendRequest = async (request: FriendRequest) => {
    ...
    try {
      ...
      const requestRef = doc(
        db,
        `${getBasePath()}/friendRequests/${request.id}`
      );
      ...
      if (senderSnap.exists()) {
        const senderData = senderSnap.data();
        const updatedSenderFriendsList = [
          ...(senderData.friendsList || []),
          currentUser.uid,
        ];
      ...
      try {
        await notificationsApi.createFriendRequestResponseNotification(
          request.fromUserId,
          userProfile?.username || "Unknown Champion",
          true, // accepted
          request.id
        ...
        console.warn(
          "Could not send acceptance notification:",
          notificationError
        );
      }
  ...
  const rejectFriendRequest = async (request: FriendRequest) => {
    ...
    try {
      ...
      try {
        ...
        console.log("✅ Friend request rejection notification sent!");
      } catch (notificationError) {
        console.warn(
      ...
      showModal({
        ...
        onConfirm: async () => {
          try {
            ...
            console.error(
              "Error in mutual friend removal:",
              mutualRemovalError
            );
  ...
  const inviteFriendToRoom = async (friendId: string) => {
    ...
    try {
        ...
        messages: [
          {
            senderId: "system",
            text: `${
              userProfile?.username || "A champion"
            } created this room and invited their friend.`,
            timestamp: Date.now(),
          },
        ],
      ...
      await addDoc(requestsRef, {
        ...
        toUserId: friendId,
        toFriendlyId:
          friends.find((f) => f.id === friendId)?.friendlyUserId || "Unknown",
        status: "pending",
        ...
        message: `${
          userProfile?.username || "A champion"
        } invited you to join their challenge room!`,
      });
      ...
      showModal({
        ...
        message:
          "You've invited your champion friend to join your challenge room!",
        type: "success",
      ...
      <div className="py-6 relative z-10">
        ...
        >
          ...
          <p className="text-gray-600 mb-4">
            Enter your friend's Champion ID to send them a friend request.
          </p>
          <div className="flex">
            <input
              type="text"
                  ...
                  <div className="flex items-center mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Champion ID:{" "}
                        {request.fromFriendlyId || request.fromUserId}
                      </p>
                    </div>
                  </div>
        ...
        >
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-purple-900">
              My Champion Alliance ({friends.length})
            </h2>
          ...
          {loading ? (
            <div className="text-center py-10">
              ...
              <p className="text-purple-600">
                Loading your legendary champions...
              </p>
            </div>
          ) : friends.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border-2 border-purple-100">
              ...
              <p className="text-purple-600 mb-6">
                Add champions to start building your alliance!
              </p>
            </div>
