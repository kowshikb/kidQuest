...
export const useGameState = (options: UseGameStateOptions = {}): UseGameStateReturn => {
  ...
  const fetchGameState = useCallback(async (isRetry = false) => {
    if (!currentUser) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const response = await gameStateApi.getInitialGameState(currentUser.uid);
      ...
      if (retryCount < maxRetries) {
        ...
        showModal({
          ...
          message: "We're having trouble loading your game data. Please check your connection and try again.",
          type: "error",
          confirmText: "Retry",
          onConfirm: () => {
            setRetryCount(0);
  ...
  const syncChanges = useCallback(async (changes: Partial<GameState>): Promise<boolean> => {
    if (!currentUser) {
    ...
    try {
      const response = await gameStateApi.syncGameState(currentUser.uid, changes);
